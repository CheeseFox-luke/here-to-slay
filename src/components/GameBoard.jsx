/**
 * GameBoard — 对局场地（原 App.jsx 主体，核心玩法 UI）
 *
 * 职责：渲染牌桌、手牌、回合与掷骰等；由 App 在 Lobby「Start Game」后挂载。
 * 不处理 Create/Join/Lobby；不修改 gameActions / gameState 规则，只调用它们。
 *
 * [Requires]（由 App 注入）
 * - playerCount: number — 来自 RoomInfo.maxPlayers，传给 initGame(playerCount)
 *
 * [Depends]（运行期必须存在，类似 Unity 场景里的其它 Manager）
 * - gameState.initGame — 初始化 GameState
 * - gameActions.* — 所有出牌/攻击/回合逻辑
 * - data/*, deckHelpers, dice, effects, cardEffects — 卡牌与效果数据
 * - components/* — 子 UI（PartyBoard、DeckPile、各类 Dialog 等）
 * - App.css — 对局布局样式
 *
 * [Provides]
 * - 完整可玩的单机 Demo 对局界面（多人在同一浏览器轮流操作）
 *
 * [Note]
 * - App 使用 key={room.roomId} 强制 remount，换房或重开时会重新 initGame
 * - playerCount 须在 gameState.initGame 合法范围内（当前 1～6）
 */
import { useEffect, useState } from 'react'
import ActiveMonsters from './ActiveMonsters.jsx'
import CardDisplay from './CardDisplay.jsx'
import DebugPanel from './DebugPanel.jsx'
import DeckPile from './DeckPile.jsx'
import CardPullDialog from './CardPullDialog.jsx'
import EffectTargetDialog from './EffectTargetDialog.jsx'
import ModifierChoiceDialog from './ModifierChoiceDialog.jsx'
import ModifierTargetDialog from './ModifierTargetDialog.jsx'
import PartyBoard from './PartyBoard.jsx'
import RollFeedback from './RollFeedback.jsx'
import SkillConfirmDialog from './SkillConfirmDialog.jsx'
...
/**
 * @param {{ playerCount: number }} props
 */
function GameBoard({ playerCount }) {
  // 对局状态根；playerCount 由 Lobby/App 传入，与房间 maxPlayers 对齐
  const [game, setGame] = useState(() => initGame(playerCount))
